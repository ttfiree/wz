...
public class ResourcesUtil {
	...
	static {
		...
		try {
			if (tmpdataDirPath != null) {
				logger.info(tmpdataDirPath);
				File tmpFileDir = new File(tmpdataDirPath);
				tmpdataDirPath = tmpFileDir.getCanonicalPath();
				if (tmpFileDir.exists()) {
					logger.info("指定数据目录：" + tmpdataDirPath);
				} else {
					logger.info("目录不存在：" + tmpdataDirPath);
					tmpdataDirPath = null;
				}
			}
		} catch (Exception e) {
	...
	private static String getProjectPath() {
		...
		java.net.URL url = ResourcesUtil.class.getProtectionDomain().getCodeSource().getLocation();
